...
**The middle-layer finding.** Probing accuracy for truthfulness-related properties peaks in middle layers (e.g., layers 12--18 in a 32-layer model), not in the final layer. This partially overlaps the L15--21 zone predicted for SAD's signal concentration, though the overlap is plausible rather than load-bearing --- [Gate 3](../instrument/gate-discipline.md) uses synthetic HMMs, not truthfulness labels. Semantic Entropy Probes similarly capture uncertainty best from mid-to-late layers. This suggests that truth-relevant representations are constructed progressively and may be partially overwritten by later processing.
...
**Dynamical systems alternatives.** State-space models (Mamba, Gu & Dao, ICLR 2024, [arXiv:2312.00752](https://arxiv.org/abs/2312.00752)) explicitly represent sequences as dynamical systems --- the same framing SAD applies post-hoc to attention. Whether Mamba's explicit state representation makes it more or less amenable to the Takens framing is an open question. SAD's approach is architecture-agnostic (it probes from the outside), but the SSM literature provides independent theoretical grounding for the dynamical systems analogy.
...
| Linear attention | Katharopoulos et al., ICML 2020 ([arXiv:2006.16236](https://arxiv.org/abs/2006.16236)) | ELU+1 feature map, linear complexity via kernel trick |
